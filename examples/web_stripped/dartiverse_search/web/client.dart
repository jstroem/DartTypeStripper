...
class Client {
  static const RECONNECT_DELAY = const Duration(milliseconds: 500);

  var connectPending = false;
  var mostRecentSearch = null;
  var webSocket;
  final log = new DivElement();
  var searchElement = querySelector('#q');
  var statusElement = querySelector('#status');
  var resultsElement = querySelector('#results');
  ...
  connect() {
    ...
    webSocket.onError.first.then((_) {
      print(
          "Failed to connect to ${webSocket.url}. " "Run bin/server.dart and try again.");
      onDisconnected();
    });
  }
  ...
  handleMessage(data) {
    ...
    switch (response) {
      ...
      case 'searchDone':
        setStatus(
            resultsElement.children.isEmpty ?
