...
final log = new Logger('DartiverseSearch');
    ...
    switch (request) {
      case 'search':
        ...
        for (var engine in searchEngines) {
          engine.search(input).listen((result) {
            // The search-engine found a result. Send it to the client.
            log.info(
                "Got result from ${engine.name} for '$input': " "${result.title}");
            var response = {
...
main() {
  ...
  var port = 9223; // TODO use args from command line to set this

  HttpServer.bind(InternetAddress.LOOPBACK_IP_V4, port).then((server) {
    log.info(
        "Search server is running on " "'http://${server.address.address}:$port/'");
    var router = new Router(server);

    // The client will connect using a WebSocket. Upgrade requests to '/ws' and
    // forward them to 'handleWebSocket'.
    router.serve(
        '/ws').transform(new WebSocketTransformer()).listen(handleWebSocket);
    ...
    router.serve("/client.dart").listen((request) {
      var clientScript = Platform.script.resolve("../web/client.dart");
