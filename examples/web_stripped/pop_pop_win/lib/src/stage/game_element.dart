...
class GameElement extends Sprite {
  ...
  static const _dartAnimationOffset =
      const Vector(-512 + 0.5 * SquareElement.SIZE, -388 + 0.5 * SquareElement.SIZE);
  ...
  var _boardElement;
  var _scoreElement;
  var _newGameButton, _logoButton;
  var _popLayer = new Sprite(),
      _dartLayer = new Sprite();

  var _boardSize, _boardScale;


  var _targetX, _targetY;
  var _animations;

  get game => manager.game;
  get resourceManager => manager.resourceManager;

  get boardSize => _boardSize;
  get boardScale => _boardScale;

  get scoreElement => _scoreElement;
  get boardElement => _boardElement;
  ...
  get canRevealTarget => _targetX != null && game.canReveal(_targetX, _targetY);
        ...
        if (game.canReveal(x, y)) {
          ...
          final adjHidden = game.field.getAdjacentIndices(x, y).map((i) {
            ...
            return new Point(t.item1, t.item2);
              ...
              (t) => game.getSquareState(t.x, t.y) == SquareState.hidden).toList();
  ...
  _startPopAnimation(start, [reveals = null]) {
    if (reveals == null) {
      assert(game.state == GameState.lost);
      reveals = new Iterable.generate(game.field.length).map((i) {
        ...
        return new Tuple(c, game.getSquareState(c.x, c.y));
          ...
          (t2) =>
              t2.item2 == SquareState.bomb ||
                  t2.item2 == SquareState.hidden).map((t2) => t2.item1).toList();
    }
    ...
    for (var v in values) {
      var c = v[0];
      var initialOffset = v[1];
      var squareOffset = v[2];
      var delay = v[3];
      ...
      var anim =
          new FlipBook(_animations.getBitmapDatas(texturePrefix), stage.frameRate, false)
          ..x = squareOffset.x
          ..y = squareOffset.y
          ..alpha = 0
          ..mouseEnabled = false
      ...
      stage.juggler
          ..add(anim)
          ..delayCall(
              () => _animationDelay(anim, se, ss),
              delay / stage.frameRate);
    }
  }

  _startDartAnimation(points) {
    ...
    for (var point in points) {
      ...
      var dart =
          new FlipBook(_animations.getBitmapDatas('dart'), stage.frameRate, false)
          ..x = squareOffset.x
          ..y = squareOffset.y
          ..mouseEnabled = false
          ..play()
      ...
      var shadow =
          new FlipBook(_animations.getBitmapDatas('shadow'), stage.frameRate, false)
          ..x = squareOffset.x
          ..y = squareOffset.y
          ..mouseEnabled = false
          ..play()
...
final _titleClickedEventHandle = new StreamController();

get titleClickedEvent => _titleClickedEventHandle.stream;
